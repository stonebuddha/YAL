...
let rec fm_to_z3 ctx formula =
	match formula with
		...
		| FmSub(fm1, fm2) -> (Arithmetic.mk_sub ctx [fm_to_z3 ctx fm1;fm_to_z3 ctx fm2])
		| FmMul(fm1, fm2) -> (Arithmetic.mk_mul ctx [fm_to_z3 ctx fm1;fm_to_z3 ctx fm2])
		| FmDiv(fm1, fm2) -> (Arithmetic.mk_div ctx (fm_to_z3 ctx fm1) (fm_to_z3 ctx fm2))
